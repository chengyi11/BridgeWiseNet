...
class ModelConfig(BaseModel):
    ...
    # WSS Adapter (DINOv3 only)
    wss_enable: bool = True
    wss_layers: List[int] = Field(default_factory=lambda: [2, 5, 8, 11])
    wss_levels: int = 1
    wss_reduction_dim: int = 64

    # PRISM auxiliary branch (training-only)
    prism_enable: bool = True
    prism_align_layer: int = -2
    prism_N: int = 32
    prism_K: int = 8
    prism_delta_ratio: float = 0.35
    prism_thickness_ratio: float = 0.25
    prism_pool_center_ratio: float = 0.25
    prism_tcn_layers: int = 2
    prism_tcn_kernel: int = 3
    prism_cross_heads: int = 8
    prism_srm_hidden: int = 128
    prism_pos_dim: int = 32
    prism_pgd_coef: float = 1.0
    prism_ce_coef: float = 1.0
