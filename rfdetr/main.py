...
#     parser.add_argument('--resolution', type=int, default=1280, help="input resolution")
...
"""
cleaned main file + unified training log (train.log)
- capture env/args snapshot
- param counts (total/trainable/frozen, DINO/DETR split)
- per-iter text lines (via tee stdout/stderr)
- per-epoch JSON (EPOCH_JSON ...)
"""
...
def setup_logging_and_tee(output_dir: Path):
    """
    Configure logging into {output_dir}/train.log and tee stdout/stderr into the same file.
    Return (logger_name, opened_file_handle, old_stdout, old_stderr).
    """
    ...
    # avoid duplicated handlers if re-entered
    ...
    # --- tee prints/progress lines into the same file ---
    ...
    return "rfdetr.run", f, old_stdout, old_stderr

def close_tee(file_handle, old_stdout, old_stderr):
    try:
    ...
    try:
        file_handle.flush(); file_handle.close()
    except Exception:
        pass

def log_environment(logger_obj, args):
    logger_obj.info("========== Environment ==========")
    logger_obj.info(f"Time       : {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger_obj.info(f"Platform   : {platform.platform()}")
    logger_obj.info(f"Python     : {platform.python_version()}")
    logger_obj.info(f"PyTorch    : {torch.__version__} (CUDA avail: {torch.cuda.is_available()})")
    if torch.cuda.is_available():
        try:
            logger_obj.info(f"CUDA       : {torch.version.cuda}")
            logger_obj.info(f"GPU        : {torch.cuda.get_device_name(torch.cuda.current_device())}")
        except Exception:
            pass
    logger_obj.info(f"Run dir    : {Path(args.output_dir).absolute()}")
    logger_obj.info(f"Encoder    : {getattr(args, 'encoder', 'N/A')}")
    logger_obj.info(f"Dataset    : {getattr(args, 'coco_path', None) or getattr(args, 'dataset_dir', None)}")
    # 参数快照（避免不可序列化对象）
    safe_args = {k: (str(v) if not isinstance(v, (int, float, str, bool, list, dict, type(None))) else v)
                 for k, v in vars(args).items()}
    logger_obj.info("Args dump  : " + json.dumps(safe_args, ensure_ascii=False, indent=2))

def _is_dino_param_name(name: str) -> bool:
    """Heuristic: names indicating the visual encoder/backbone (DINO/DINOv2/ViT etc.)."""
    prefixes = ('backbone', 'encoder', 'visual_encoder', 'img_backbone', 'vit', 'patch_embed')
    return any(name.startswith(p) or (f'.{p}.' in name) for p in prefixes)

def count_and_log_params(logger_obj, model: torch.nn.Module):
    total = 0
    trainable = 0
    dino_total = dino_tr = 0
    detr_total = detr_tr = 0
    ...
    for n, p in model.named_parameters():
        ...
        if p.requires_grad:
            trainable += n_params
        if _is_dino_param_name(n):
            dino_total += n_params
            if p.requires_grad: dino_tr += n_params
        else:
            detr_total += n_params
            if p.requires_grad: detr_tr += n_params

    frozen = total - trainable
    logger_obj.info("========== Parameters ==========")
    logger_obj.info(f"Total params     : {total:,}")
    logger_obj.info(f"Trainable params : {trainable:,}")
    logger_obj.info(f"Frozen params    : {frozen:,}")
    logger_obj.info(f"DINO total/train : {dino_total:,} / {dino_tr:,} (frozen {dino_total - dino_tr:,})")
    logger_obj.info(f"DETR total/train : {detr_total:,} / {detr_tr:,} (frozen {detr_total - detr_tr:,})")
    # JSON one-liner for scripts
    logger_obj.info("PARAM_JSON " + json.dumps({
        "total": total, "trainable": trainable, "frozen": frozen,
        "dino_total": dino_total, "dino_trainable": dino_tr, "dino_frozen": dino_total - dino_tr,
        "detr_total": detr_total, "detr_trainable": detr_tr, "detr_frozen": detr_total - detr_tr
    }))
...
class Model:
    ...
    def train(self, callbacks: DefaultDict[str, List[Callable]], **kwargs):
        ...
        # ---- setup logging & tee (rank0 only) ----
        tee_f = None
        old_stdout = old_stderr = None
        if utils.is_main_process():
            run_logger_name, tee_f, old_stdout, old_stderr = setup_logging_and_tee(Path(args.output_dir))
            run_logger = logging.getLogger(run_logger_name)
            run_logger.info("Logging initialized. Log file: %s", str(Path(args.output_dir) / "train.log"))
            run_logger.info("Run dir: %s", str(Path(args.output_dir)))
            log_environment(run_logger, args)
        else:
            run_logger = logging.getLogger("rfdetr.run")
        ...
        # parameter counts (once)
        if utils.is_main_process():
            try:
                count_and_log_params(run_logger, model_without_ddp)
            except Exception as e:
                logging.getLogger("rfdetr.run").warning(f"Param counting failed: {e}")
        ...
        if args.eval:
            ...
            if args.output_dir:
                utils.save_on_master(coco_evaluator.coco_eval["bbox"].eval, output_dir / "eval.pth")
            # close tee before return
            if utils.is_main_process() and tee_f is not None:
                close_tee(tee_f, old_stdout, old_stderr)
            return
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if utils.is_main_process():
                logging.getLogger("rfdetr.run").info(f"===== EPOCH {epoch} START =====")
            ...
            # === Unified epoch JSON into train.log ===
            if utils.is_main_process():
                logging.getLogger("rfdetr.run").info("EPOCH_JSON " + json.dumps(log_stats, ensure_ascii=False))

                # for evaluation logs (remain as files)
                if coco_evaluator is not None:
                    (output_dir / 'eval').mkdir(exist_ok=True)
                    if "bbox" in coco_evaluator.coco_eval:
                        filenames = ['latest.pth']
                ...
                logging.getLogger("rfdetr.run").info(f"===== EPOCH {epoch} END | time={epoch_time_str} =====")
        ...
        # --- always close tee at the very end (rank0) ---
        if utils.is_main_process() and tee_f is not None:
            close_tee(tee_f, old_stdout, old_stderr)
...
def get_args_parser():
    ...
    # DINOv3 loading (optional)
    parser.add_argument('--dinov3_repo_dir', type=str, default=None,
                        help='Path to local dinov3 repo for torch.hub (if not using HF).')
    parser.add_argument('--dinov3_weights_path', type=str, default=None,
                        help='Path/URL to dinov3 weights for torch.hub (if not using HF).')
    parser.add_argument('--dinov3_hf_token', type=str, default=None,
                        help='HuggingFace token (or set HUGGINGFACE_HUB_TOKEN).')
    parser.add_argument('--dinov3_prefer_hf', dest='dinov3_prefer_hf', action='store_true',
                        help='Prefer loading DINOv3 from HuggingFace when possible.')
    parser.add_argument('--dinov3_no_hf', dest='dinov3_prefer_hf', action='store_false',
                        help='Disable HuggingFace loading for DINOv3 (force torch.hub).')
    parser.set_defaults(dinov3_prefer_hf=True)

    # WSS Adapter (default ON)
    parser.add_argument('--wss_enable', dest='wss_enable', action='store_true',
                        help='Enable WSS adapter in DINOv3 backbone (default: on).')
    parser.add_argument('--wss_disable', dest='wss_enable', action='store_false',
                        help='Disable WSS adapter.')
    parser.set_defaults(wss_enable=True)
    parser.add_argument('--wss_layers', type=int, nargs='+', default=[2, 5, 8, 11],
                        help='Insert WSS after these ViT block indices (0-based).')
    parser.add_argument('--wss_levels', type=int, default=1,
                        help='Haar wavelet levels for WSS (default: 1).')
    parser.add_argument('--wss_reduction_dim', type=int, default=64,
                        help='Bottleneck channels inside WSS (default: 64).')

    # PRISM auxiliary branch (default ON; training-only)
    parser.add_argument('--prism_enable', dest='prism_enable', action='store_true',
                        help='Enable PRISM auxiliary branch (default: on).')
    parser.add_argument('--prism_disable', dest='prism_enable', action='store_false',
                        help='Disable PRISM auxiliary branch.')
    parser.set_defaults(prism_enable=True)
    parser.add_argument('--prism_align_layer', type=int, default=-2,
                        help='Which decoder layer (hs index) to take query features from for PRISM alignment.')
    parser.add_argument('--prism_N', type=int, default=32, help='Axial samples N.')
    parser.add_argument('--prism_K', type=int, default=8, help='Normal-direction samples K per band.')
    parser.add_argument('--prism_delta_ratio', type=float, default=0.35, help='Band offset ratio to short side.')
    parser.add_argument('--prism_thickness_ratio', type=float, default=0.25, help='Band thickness ratio to short side.')
    parser.add_argument('--prism_pool_center_ratio', type=float, default=0.25, help='Center window ratio for pooling.')
    parser.add_argument('--prism_tcn_layers', type=int, default=2, help='Number of TCN blocks.')
    parser.add_argument('--prism_tcn_kernel', type=int, default=3, help='TCN kernel size.')
    parser.add_argument('--prism_cross_heads', type=int, default=8, help='Cross-attention heads.')
    parser.add_argument('--prism_srm_hidden', type=int, default=128, help='SRM hidden dim.')
    parser.add_argument('--prism_pos_dim', type=int, default=32, help='SRM position embedding dim.')
    parser.add_argument('--prism_pgd_coef', type=float, default=1.0, help='Weight for PRISM PGD loss.')
    parser.add_argument('--prism_ce_coef', type=float, default=1.0, help='Weight for PRISM CE loss.')
...
def populate_args(
    ...
    prism_N=32,
    prism_K=8,
    prism_delta_ratio=0.35,
    prism_thickness_ratio=0.25,
    prism_pool_center_ratio=0.25,
    prism_tcn_layers=2,
    prism_tcn_kernel=3,
    prism_cross_heads=8,
    prism_srm_hidden=128,
    prism_pos_dim=32,
    prism_pgd_coef=1.0,
    prism_ce_coef=1.0,
    ...
    **extra_kwargs  # To handle any unexpected arguments
    ...
    args = argparse.Namespace(
        ...
        prism_enable=prism_enable,
        prism_align_layer=prism_align_layer,
        prism_N=prism_N,
        prism_K=prism_K,
        prism_delta_ratio=prism_delta_ratio,
        prism_thickness_ratio=prism_thickness_ratio,
        prism_pool_center_ratio=prism_pool_center_ratio,
        prism_tcn_layers=prism_tcn_layers,
        prism_tcn_kernel=prism_tcn_kernel,
        prism_cross_heads=prism_cross_heads,
        prism_srm_hidden=prism_srm_hidden,
        prism_pos_dim=prism_pos_dim,
        prism_pgd_coef=prism_pgd_coef,
        prism_ce_coef=prism_ce_coef,
        dec_layers=dec_layers,
        dim_feedforward=dim_feedforward,
        hidden_dim=hidden_dim,
        sa_nheads=sa_nheads,
