...
from rfdetr.models.prism import PRISMTrunk
from rfdetr.models.prism.trunk import PRISMConfig
...
class LWDETR(nn.Module):
    ...
    def __init__(self,
                 ...
                 bbox_reparam=False,
                 prism_enable: bool = False,
                 prism_align_layer: int = -2,
        ...
        # -----------------------------
        # PRISM Trunk (training-only auxiliary branch)
        # -----------------------------
        self.prism_enable = prism_enable
        self.prism_align_layer = prism_align_layer
        if self.prism_enable:
            cfg = prism_cfg if prism_cfg is not None else PRISMConfig()
            if isinstance(cfg, dict):
                cfg = PRISMConfig(**cfg)
            self.prism_trunk = PRISMTrunk(hidden_dim=hidden_dim, num_classes=num_classes, cfg=cfg)
        else:
            self.prism_trunk = None
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        # -----------------------------
        # PRISM auxiliary branch (training only)
        # -----------------------------
        if (
            self.training
            and self.prism_enable
            and self.prism_trunk is not None
            and targets is not None
            and len(srcs) > 0
            and hs is not None
        ):
            prism_out = self.prism_trunk(srcs[0], samples, targets)
            out['prism_query_embed'] = hs[self.prism_align_layer]  # [B, Q, 256]
            out['prism_gt_embed'] = prism_out['prism_embed']        # list([n_gt, 256])
            out['prism_gt_logits'] = prism_out['prism_logits']      # list([n_gt, num_classes])
...
class SetCriterion(nn.Module):
    ...
    # -----------------------------
    # PRISM losses (auxiliary, training-only)
    # -----------------------------
    def loss_prism_pgd(self, outputs, targets, indices, num_boxes):
        """Profile-guided distillation (PGD) between matched queries and PRISM GT embeddings.

        - queries come from a chosen decoder layer (e.g., hs[-2])
        - teacher GT embeddings come from PRISM trunk applied on srcs[0]
        Teacher embeddings are detached to prevent trivial collapse.
        """
        if 'prism_query_embed' not in outputs or 'prism_gt_embed' not in outputs:
            return {}
        prism_query = outputs['prism_query_embed']  # [B,Q,C]
        prism_gt_list = outputs['prism_gt_embed']    # list([n_gt,C])
        if prism_query is None or prism_gt_list is None:
            return {}
        ...
        for b, (src_idx, tgt_idx) in enumerate(indices):
            if src_idx.numel() == 0:
                continue
            if b >= len(prism_gt_list):
                continue
            gt_b = prism_gt_list[b]
            if gt_b is None or gt_b.numel() == 0:
                continue
            # gather matched query features and matched GT prism embeddings
            q = prism_query[b, src_idx]  # [M,C]
            g = gt_b[tgt_idx].detach()   # [M,C] (detach teacher)
            qs.append(q)
            gts.append(g)

        if len(qs) == 0:
            # no matches in the batch
            return {'loss_prism_pgd': prism_query.sum() * 0.0}

        q_all = torch.cat(qs, dim=0)
        g_all = torch.cat(gts, dim=0)
        q_all = F.normalize(q_all, dim=-1)
        g_all = F.normalize(g_all, dim=-1)
        loss = 1.0 - (q_all * g_all).sum(dim=-1)
        return {'loss_prism_pgd': loss.sum() / max(num_boxes, 1.0)}

    def loss_prism_ce(self, outputs, targets, indices, num_boxes):
        """CE on PRISM GT embeddings (object-only)."""
        if 'prism_gt_logits' not in outputs:
            return {}
        prism_logits_list = outputs['prism_gt_logits']
        if prism_logits_list is None:
            return {}

        logits_all = []
        labels_all = []
        for b, tgt in enumerate(targets):
            if b >= len(prism_logits_list):
                continue
            lg = prism_logits_list[b]
            if lg is None or lg.numel() == 0:
                continue
            labels = tgt.get('labels', None)
            if labels is None or labels.numel() == 0:
                continue
            # ensure length match (PRISM trunk uses GT boxes)
            n = min(lg.shape[0], labels.shape[0])
            logits_all.append(lg[:n])
            labels_all.append(labels[:n])

        if len(logits_all) == 0:
            # no GT in batch
            dummy = outputs['pred_logits'].sum() * 0.0
            return {'loss_prism_ce': dummy}

        logits_all = torch.cat(logits_all, dim=0)
        labels_all = torch.cat(labels_all, dim=0)
        # standard CE over class ids (no "no-object" targets here)
        loss = F.cross_entropy(logits_all, labels_all)
        return {'loss_prism_ce': loss}
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        loss_map = {
            ...
            'prism_pgd': self.loss_prism_pgd,
            'prism_ce': self.loss_prism_ce,
        }
...
def build_model(args):
    ...
    backbone = build_backbone(
        ...
        # WSS adapter
        wss_enable=getattr(args, 'wss_enable', False),
        wss_layers=getattr(args, 'wss_layers', None),
        wss_levels=getattr(args, 'wss_levels', 1),
        wss_reduction_dim=getattr(args, 'wss_reduction_dim', 64),
    )
    ...
    prism_cfg = None
    if getattr(args, 'prism_enable', False):
        prism_cfg = PRISMConfig(
            N=getattr(args, 'prism_N', 32),
            K=getattr(args, 'prism_K', 8),
            delta_ratio=getattr(args, 'prism_delta_ratio', 0.35),
            thickness_ratio=getattr(args, 'prism_thickness_ratio', 0.25),
            pool_center_ratio=getattr(args, 'prism_pool_center_ratio', 0.25),
            tcn_layers=getattr(args, 'prism_tcn_layers', 2),
            tcn_kernel=getattr(args, 'prism_tcn_kernel', 3),
            cross_heads=getattr(args, 'prism_cross_heads', 8),
            srm_hidden=getattr(args, 'prism_srm_hidden', 128),
            pos_dim=getattr(args, 'prism_pos_dim', 32),
        )
    ...
    model = LWDETR(
        ...
        prism_enable=getattr(args, 'prism_enable', False),
        prism_align_layer=getattr(args, 'prism_align_layer', -2),
        prism_cfg=prism_cfg,
    )
    return model
...
def build_criterion_and_postprocessors(args):
    ...
    # PRISM auxiliary losses (training-time distillation)
    if getattr(args, 'prism_enable', False):
        weight_dict['loss_prism_pgd'] = float(getattr(args, 'prism_pgd_coef', 1.0))
        weight_dict['loss_prism_ce'] = float(getattr(args, 'prism_ce_coef', 1.0))
        losses += ['prism_pgd', 'prism_ce']
